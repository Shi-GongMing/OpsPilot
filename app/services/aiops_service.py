...
from app.core.checkpoint_manager import get_checkpointer
from app.services.diagnosis_archiver import archive_diagnosis
...
class AIOpsService:
    ...
    async def _ensure_checkpointer(self):
        """确保 checkpointer 已初始化（首次调用时重建带持久化的图）"""
    ...
    def _build_graph(self):
        ...
        def should_continue(state: PlanExecuteState) -> str:
            ...
            logger.info("计划执行完毕，返回 replanner 生成响应")
    ...
    async def execute(
        ...
        try:
            ...
            # 获取最终状态（使用异步版本兼容 AsyncSqliteSaver）
            final_state = await self.graph.aget_state(config_dict)
            final_response = ""
            final_past_steps: list = []
            ...
            # 归档诊断结果到向量库（线程池执行，不阻塞主流程）
            if final_response and len(final_response.strip()) > 50:
                import asyncio
                asyncio.create_task(
                    asyncio.to_thread(
                        archive_diagnosis, session_id, user_input, final_past_steps, final_response
                    )
                )
